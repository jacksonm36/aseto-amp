...
def main():
    ...
    with open(wrapper, "w", encoding="utf-8", newline="\n") as handle:
        ...
        handle.write('    amplog "Monitor Info" "Info" "=== Port check: $LABEL ==="\n')
        ...
        handle.write('    [ -n "$TCP_S" ] && amplog "Monitor Info" "Info" "TCP ${GP}: LISTENING ($TCP_S)" || amplog "Monitor Warning" "Warning" "TCP ${GP}: NOT listening"\n')
        handle.write('    [ -n "$UDP_S" ] && amplog "Monitor Info" "Info" "UDP ${GP}: LISTENING ($UDP_S)" || amplog "Monitor Warning" "Warning" "UDP ${GP}: NOT listening"\n')
        handle.write('    [ -n "$HTTP_S" ] && amplog "Monitor Info" "Info" "HTTP ${HP}: LISTENING ($HTTP_S)" || amplog "Monitor Warning" "Warning" "HTTP ${HP}: NOT listening"\n')
        handle.write('    CONNS=$(ss -tnp 2>/dev/null | grep ":${GP} " | wc -l)\n')
        handle.write('    amplog "Monitor Info" "Info" "Active connections on ${GP}: ${CONNS}"\n')
        handle.write('    PROC=$(ps aux 2>/dev/null | grep -c "[A]ssettoCorsaEVO")\n')
        handle.write('    amplog "Monitor Info" "Info" "Server processes: ${PROC}"\n')
        handle.write('  }\n')
        handle.write('  sleep 10 && check_ports "10s after launch"\n')
        handle.write('  sleep 10 && check_ports "20s after launch"\n')
        handle.write('  sleep 30 && check_ports "50s after launch"\n')
        handle.write('  sleep 60 && check_ports "110s after launch (pre-heartbeat)"\n')
        handle.write('  sleep 30 && check_ports "140s after launch (post-heartbeat)"\n')
        handle.write(') &\n')
